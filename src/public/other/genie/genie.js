...
(function(window, undefined){
	...
	var document = window.document,
		...
		prefixedEvent = function(el, type, callback, capture) {
			var pfx = 'webkit moz ms o standard '.split(' ');
			for (var p=0, pl=pfx.length; p<pl; p++) {
				...
				if(type=='TransitionEnd' && pfx[p]=='standard'){
					el.addEventListener('transitonend', callback, capture);
	...
	GenieInit.prototype = {
			...
			switch(this.animationDirection){
				case 'top':
					...
					for (i=-off; i<step_length_top - off; i++) {
						...
						htm += '<div class="genie-step gen-col" style="left: 0px; top: '+ top +'px; width: '+ source_dim.w +
				...
				case 'right':
					...
					for (; i<step_length_right; i++) {
						...
						htm += '<div class="genie-step gen-col" style="left: ' + left + 'px; top: 0px; width: '+ (step_quantum + 1) +
		...
		expand: function(source,callback2) {
			var step_quantum = this.step_quantum,
			...
			switch(this.animationDirection){
				case 'bottom':
					...
					for (; i<step_length_bottom; i++) {
						htm += '<div class="genie-step gen-exp" style="top: '+ (i * step_quantum) +
					...
					//prefixedEvent(fixed.lastChild, 'TransitionEnd', delegate.call(this, callback2));
				...
				case 'top':
					...
					for (i=-off; i<step_length_top-off; i++, k--) {
						htm += '<div class="genie-step gen-exp" style="top: '+ (k * step_quantum) +
				...
				case 'right':
					...
					for (; i<step_length_right; i++) {
						htm += '<div class="genie-step gen-exp" style="left: '+ (i * step_quantum) +
				...
				case 'left':
					...
					for (i=-off; i<step_length_left-off; i++, k--) {
						htm += '<div class="genie-step gen-exp" style="left: '+ (k * step_quantum) +
	...
	var genie = {
		expand: function(source, target, directions, quantum, callback,callback2){
